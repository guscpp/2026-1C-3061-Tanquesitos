...
{
    // 1. Sumamos el TextureOffset a las coordenadas de textura recibidas
    float2 uv = input.TextureCoordinate + TextureOffset;

    // 2. Usamos esas nuevas coordenadas para leer la textura
    float4 texColor = tex2D(textureSampler, uv);

    // 3. Devolvemos el color multiplicado por el color difuso
