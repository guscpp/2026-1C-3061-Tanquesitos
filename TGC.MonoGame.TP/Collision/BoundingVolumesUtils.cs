...
namespace TGC.MonoGame.TP.Collisions
{
    ...
    /// </summary>
    public static class BoundingVolumesUtils
    {
        /// <summary>
        ...
        /// <summary>
        ///     Scales a vector3 given certain scalar.
        /// </summary>
        /// <param name="scale">The scale for every axis</param>
        /// <param name="box">The <see cref="BoundingBox"/> to scale</param>
        /// <returns>A new box with its extents scaled</returns>
        ...
        public static BoundingSphere Scale(BoundingSphere sphere, float scale)
        {
            ...
            return new BoundingSphere(sphere.Center, scaledRadius);
        }
        ...
        public static BoundingBox CreateAABB(Model model)
        {
            ...
            for (int index = 0; index < meshes.Count; index++)
            {
                ...
                for (int subIndex = 0; subIndex < meshParts.Count; subIndex++)
                {
                    ...
                    for (var vertexIndex = 0; vertexIndex < rawVertexBuffer.Length; vertexIndex += vertexSize)
                    {
                        ...
                        var vertex = new Vector3(rawVertexBuffer[vertexIndex],
                                        rawVertexBuffer[vertexIndex + 1],
                                        rawVertexBuffer[vertexIndex + 2]);
        ...
        public static BoundingBox CreateAABBFromModel(Model model)
        {
            ...
            foreach (var mesh in model.Meshes)
            {
                ...
                // aplicar la transformacion del hueso padre al centro
                var center = Vector3.Transform(sphere.Center, transforms[mesh.ParentBone.Index]);

                min = Vector3.Min(min, center - Vector3.One * sphere.Radius);
                max = Vector3.Max(max, center + Vector3.One * sphere.Radius);
            }
